...
Arguably one of the main reasons for the success of [EIP-20](./00020.md) tokens lies in the interplay between `approve` and `transferFrom`, which allows for tokens to not only be transferred between externally owned accounts (EOA), but to be used in other contracts under application specific conditions by abstracting away `msg.sender` as the defining mechanism for token access control.
...
For an improved user experience, the signed data is structured following [EIP-712](./00712.md), which already has wide spread adoption in major RPC providers.
...
So far, many of these proposals have seen very little adoption, and the ones that have been adopted (such as [EIP-777](./00777.md)), introduce a lot of additional functionality, causing unexpected behavior in mainstream contracts.
