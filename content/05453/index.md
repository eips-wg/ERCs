...
This EIP establish a general protocol for permitting approving function calls in the same transaction rely on [ERC-5750](../05750.md).
Unlike a few prior art ([ERC-2612](../02612.md) for [ERC-20](../00020.md), `ERC-4494` for [ERC-721](../00721.md) that
...
See [`IERC5453.sol`](./assets/IERC5453.sol).

### Behavior specification

As specified in [ERC-5750 General Extensibility for Method Behaviors](../05750.md), any compliant method that has an `bytes extraData` as its
last method designated for extending behaviors can conform to [ERC-5453](../05453.md) as the way to indicate a permit from certain user.

1. Any compliant method of this EIP MUST be a [ERC-5750](../05750.md) compliant method.
...
8. Each `bytes sig` MUST be an ECDSA (secp256k1) signature using private key of signer whose corresponding address is `endorserAddress` signing `validityDigest` which is the a hashTypeDataV4 of [EIP-712](../00712.md) of hashStruct of `ValidityBound` data structure as followed:
...
The design assumes a `bytes calldata extraData` to maximize the flexibility of future extensions. This assumption is compatible with [ERC-721](../00721.md), [ERC-1155](../01155.md) and many other ERC-track EIPs. Those that aren't, such as [ERC-20](../00020.md), can also be updated to support it, such as using a wrapper contract or proxy upgrade.
...
See [`AERC5453.sol`](./assets/AERC5453.sol)
...
See [`EndorsableERC721.sol`](./assets/EndorsableERC721.sol)
...
See [`ThresholdMultiSigForwarder.sol`](./assets/ThresholdMultiSigForwarder.sol)
