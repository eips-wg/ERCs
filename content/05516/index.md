...
## Motivation
This EIP was inspired by the main characteristics of the [EIP-1155](../01155.md) token and by articles in which benefits and potential use cases of Soulbound/Accountbound Tokens (SBTs) were presented. 
...
- Partially compatible with [EIP-1155](../01155.md)
- Double Signature
- Multi-Token
- Multi-Owner
- Semi-Fungible
...
**Smart contracts implementing this EIP MUST implement the [EIP-165](../00165.md) `supportsInterface` function and and MUST return the constant value `true` if `0x8314f22b` is passed through the `interfaceID` argument. They also MUST implement the [EIP-1155](../01155.md) Interface and MUST return the constant value `true` if `0xd9b67a26` is passed through the `interfaceID` argument. Furthermore, they MUST implement the [EIP-1155](../01155.md) Metadata interface, and MUST return the constant value `true` if `0x0e89341c` is passed through the `interfaceID` argument.**

_See [EIP-1155](../01155.md#specification)_
...
We believe that Soulbound Tokens serve as a specialized subset of existing [EIP-1155](../01155.md) tokens. The advantage of such a design is the seamless compatibility of SBTs with existing NFT services. Service providers can treat SBTs like NFTs and do not need to make drastic changes to their existing codebase.

Making the standard mostly compatible with [EIP-1155](../01155.md) also allows for SBTs to bind to multiple addresses and to Smart Contracts.
...
### Metadata.
The [EIP-1155](../01155.md#metadata) Metadata Interface was implemented for further compatibility with [EIP-1155](../01155.md).
...
_Quoted from [EIP-1155](../01155.md#guaranteed-log-trace)_
...
**Given the multi-owner characteristic of [EIP-1155](../01155.md) compliant interfaces and contracts, SBTs will be able to bind to multiple accounts, providing a potential solution to the issue.**

Multi-owner SBTs can also be issued to a contract account that implements a multi-signature functionality (As recommended in [EIP-4973](../04973.md#exception-handling)); this can be achieved via the [EIP-1155](../01155.md#erc-1155-token-receiver) Token Receiver interface.
...
You can find an implementation of this standard in [../assets/EIP-5516](./assets/ERC5516.sol).
