...
An [EIP-5792](./05792.md) compliant capability that allows wallets to indicate to apps that they have access to funds beyond those that can be accounted for by looking up balances onchain given the wallet's address.

A wallet's ability to access auxiliary funds is communicated to apps as part of its response to an [EIP-5792](./05792.md) `wallet_getCapabilities` request. The following standard does not specify the source of these auxiliary funds, but some examples are:
...
One new [EIP-5792](./05792.md) wallet capability is defined.
...
In this specification, a chain's native asset (e.g. Ether on Ethereum) MUST be represented by "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE" as specified by [EIP-7528](./07528.md).
...
When a wallet indicates support for the `auxiliaryFunds` capability (i.e., `supported: true`), applications that use the [`wallet_sendCalls`](./05792.md#wallet-sendcalls) method MAY include a `requiredAssets` parameter in the `capabilities.auxiliaryFunds` object to enable the wallet to leverage this capability.
...
> **Note**: The `capabilities` object is specified in [EIP-5792](./05792.md) where `wallet_sendCalls` is first defined: "The capabilities field is how an app can communicate with a wallet about capabilities a wallet supports. For example, this is where an app can specify a paymaster service URL from which an [ERC-4337](./04337.md) wallet can request a paymasterAndData input for a user operation."
...
- **[ERC-20](./00020.md)**: Requires `amount` field only
- **[ERC-721](./00721.md)**: Requires both `amount` and `tokenId` fields
- **[ERC-1155](./01155.md)**: Requires both `amount` and `tokenId` fields

The `amount` field semantics vary by token standard:

- **[ERC-20](./00020.md)**: Represents token quantity in smallest unit (e.g., wei for 18-decimal tokens)
- **[ERC-721](./00721.md)**: Represents NFT quantity (typically "0x01" for single NFT instances)
- **[ERC-1155](./01155.md)**: Represents quantity of the specified token ID
