...
1. The relying party generates a SIWE Message and prefixes the SIWE Message with `\x19Ethereum Signed Message:\n<length of message>` as defined in [ERC-191](../00191.md).
2. The wallet presents the user with a structured plaintext message or equivalent interface for signing the SIWE Message with the [ERC-191](../00191.md) signed data format.
3. The signature is then presented to the relying party, which checks the signature's validity and SIWE Message content.
4. The relying party might further fetch data associated with the Ethereum address, such as from the Ethereum blockchain (e.g., ENS, account balances, [ERC-20](../00020.md)/[ERC-721](../00721.md)/[ERC-1155](../01155.md) asset ownership), or other data sources that might or might not be permissioned.
...
- `address` REQUIRED. The Ethereum address performing the signing. Its value SHOULD be conformant to mixed-case checksum address encoding specified in [ERC-55](../00055.md) where applicable.
...
- `chain-id` REQUIRED. The [EIP-155](../00155.md) Chain ID to which the session is bound, and the network where Contract Accounts MUST be resolved.
...
- For Externally Owned Accounts (EOAs), the verification method specified in [ERC-191](../00191.md) MUST be used.

- For Contract Accounts,
    - The verification method specified in [ERC-1271](../01271.md) SHOULD be used, and if it is not, the implementer MUST clearly define the verification method to attain security and interoperability for both wallets and relying parties.
    - When performing [ERC-1271](../01271.md) signature verification, the contract performing the verification MUST be resolved from the specified `chain-id`.
    - Implementers SHOULD take into consideration that [ERC-1271](../01271.md) implementations are not required to be pure functions, and can return different results for the same inputs depending on blockchain state. This can affect the security model and session validation rules. For example, a service with [ERC-1271](../01271.md) signing enabled could rely on webhooks to receive notifications when state affecting the results is changed. When it receives a notification, it invalidates any matching sessions.
...
- Wallet implementers SHOULD warn users if the substring `"wants you to sign in
  with your Ethereum account"` appears anywhere in an [ERC-191](../00191.md) message signing
...
- The origin SHOULD be read from a trusted data source such as the browser window or over WalletConnect ([ERC-1328](../01328.md)) sessions for comparison against the signing message contents.
...
- Why [ERC-191](../00191.md) (Signed Data Standard) over [EIP-712](../00712.md) (Ethereum typed structured data hashing and signing)
    - [ERC-191](../00191.md) is already broadly supported across wallets UX, while [EIP-712](../00712.md) support for friendly user display is pending. **(1, 2, 3, 4)**
    - [ERC-191](../00191.md) is simple to implement using a pre-set prefix prior to signing, while [EIP-712](../00712.md) is more complex to implement requiring the further implementations of a bespoke Solidity-inspired type system, RLP-based encoding format, and custom keccak-based hashing scheme. **(2)**
    - [ERC-191](../00191.md) produces more human-readable messages, while [EIP-712](../00712.md) creates signing outputs for machine consumption, with most wallets not displaying the payload to be signed in a manner friendly to humans. **(1)**![](./assets/signing.png)

    - [EIP-712](../00712.md) has the advantage of on-chain representation and on-chain verifiability, such as for their use in metatransactions, but this feature is not relevant for the specification's scope. **(2)**
...
- Possible future support for [EIP-712](../00712.md).
- Version interpretation rules, e.g., sign with minor revision greater than understood, but not greater major version.

## Backwards Compatibility

- Most wallet implementations already support [ERC-191](../00191.md), so this is used as a base pattern with additional features.
...
A reference implementation is available [here](./assets/example.js).
...
- Certain wallets can use smart contracts and multisigs to provide an enhanced user experience with respect to key usage and key recovery, and these can be supported via [ERC-1271](../01271.md) signing.
...
- If an [ERC-1271](../01271.md) implementation or dependent data changes the signature computation, the server SHOULD invalidate sessions appropriately.
