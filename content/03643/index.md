...
- **MUST** be [ERC-20](../00020.md) compatible.
...
The `IAgentRole` interface allows for the addition and removal of agents, as well as checking if an address is an agent. In this standard, it is the owner role, as defined by [ERC-173](../00173.md), that has the responsibility of appointing and removing agents. Any contract that fulfills the role of a Token contract or an Identity Registry within the context of this standard must be compatible with the `IAgentRole` interface.
...
contains the claims (see [Claim Holder](./assets/ONCHAINID/IERC735.sol)) required in the `Claim Topics Registry` and 
...
A detailed description of the functions can be found in the [interfaces folder](./assets/interfaces/IERC3643.sol).
...
A detailed description of the functions can be found in the [interfaces folder](./assets/interfaces/IIdentityRegistry.sol).

Note that [`IClaimIssuer`](./assets/ONCHAINID/IClaimIssuer.sol) and [`IIdentity`](./assets/ONCHAINID/IIdentity.sol) are needed in this interface as they are required for the Identity eligibility checks.
...
A detailed description of the functions can be found in the [interfaces folder](./assets/interfaces/IIdentityRegistryStorage.sol).
...
A detailed description of the functions can be found in the [interfaces folder](./assets/interfaces/ICompliance.sol).
...
The Trusted Issuer's Registry stores the contract addresses ([IClaimIssuer](./assets/ONCHAINID/IClaimIssuer.sol)) of all the trusted claim issuers for a specific security token. The Identity contract ([IIdentity](./assets/ONCHAINID/IIdentity.sol)) of token owners (the investors) must have claims signed by the claim issuers stored in this smart contract in order to be able to hold the token.
...
A detailed description of the functions can be found in the [interfaces folder](./assets/interfaces/ITrustedIssuersRegistry.sol).
...
The Claim Topics Registry stores all the trusted claim topics for the security token. The Identity contract ([IIdentity](./assets/ONCHAINID/IIdentity.sol)) of token owners must contain claims of the claim topics stored in this smart contract.
...
A detailed description of the functions can be found in the [interfaces folder](./assets/interfaces/IClaimTopicsRegistry.sol).
...
and should be able to interact with a [Claim Holder contract](./assets/ONCHAINID/IERC735.sol) to validate 
the claims linked to an [Identity contract](./assets/ONCHAINID/IIdentity.sol).
