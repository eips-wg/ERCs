...
This EIP introduces the `evalEIP712Buffer` function, which takes an [EIP-712](../00712.md) buffer and returns a human-readable text description.

## Motivation

The use case of Web3 off-chain signatures intended to be used within on-chain transaction is gaining traction and being used in multiple leading protocols (e.g. OpenSea) and standards [EIP-2612](../02612.md), mainly as it offers a fee-less experience.
...
While [EIP-712](../00712.md) originally declared in its title that being ”humanly readable” is one of its goals, it did not live up to its promise eventually and EIP-712 messages are not understandable by an average user.
...
![](./assets/media/MiceyMask-non-compliant.png)

to a much clearer user experience:

![](./assets/media/ZenGo-EIP-compliant-warning.png)
...
A reference implementation can be found [here](./assets/implementation/src/MyToken/MyToken.sol).
This toy example shows how an [EIP-20](../00020.md) contract supporting this EIP implements an EIP-712 support for "transferWithSig" functionality (a non-standard variation on Permit, as the point of this EIP is to allow readability to non-standard EIP-712 buffers).
To illustrate the usability of this EIP to some real world use case, a helper function for the actual OpenSea's SeaPort EIP-712 is implemented too in [here](./assets/implementation/src/SeaPort/SeaPort712ParserHelper.sol).
