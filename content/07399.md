...
Under this standard a flash loan is a loan of an `amount` of an [ERC-20](./00020.md) `asset` from a `lender`. This loan can remain open for the span of a single `flash` call in the `lender`.
...
A `lender` MUST implement the [ERC-7399](./07399.md) interface.
...
This EIP is a successor of [ERC-3156](./03156.md). While not directly backwards compatible, a wrapper contract offering this proposal's external interface with liquidity obtained from an ERC-3156 flash `lender` is trivial to implement.
