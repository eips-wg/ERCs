...
In contrast to EOA signatures, [EIP-1271](./01271.md) signatures are not necessarily idempotent; they can become invalid at any point in time. This poses a challenge to protocols that rely on signatures remaining valid for extended periods of time.
...
1) Try validating the signature using [EIP-1271](./01271.md); if the signature is valid, then the signature can be used as-is.
...
4) Try validating the new signature using [EIP-1271](./01271.md); if the signature is valid, it can be used as a drop-in replacement of the original signature.
