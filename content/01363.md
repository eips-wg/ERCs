...
## Simple Summary
Defines a token interface for [ERC-20](./00020.md) tokens that supports executing recipient code after `transfer` or `transferFrom`, or spender code after `approve`.
...
## Motivation
There is no way to execute code after a [ERC-20](./00020.md) transfer or approval (i.e. making a payment), so to make an action it is required to send another transaction and pay GAS twice.
...
There are many proposed uses of Ethereum smart contracts that can accept [ERC-20](./00020.md) payments. 
...
This proposal has been inspired by the [ERC-721](./00721.md) `onERC721Received` and `ERC721TokenReceiver` behaviours. 

## Specification
Implementing contracts **MUST** implement the [ERC-1363](./01363.md) interface as well as the [ERC-20](./00020.md) and [ERC-165](./00165.md) interfaces.
...
## Rationale
The choice to use `transferAndCall`, `transferFromAndCall` and `approveAndCall` derives from the [ERC-20](./00020.md) naming. They want to highlight that they have the same behaviours of `transfer`, `transferFrom` and `approve` with the addition of a callback on receiver or spender.

## Backwards Compatibility
This proposal has been inspired also by [ERC-223](https://github.com/ethereum/EIPs/issues/223) and [ERC-677](https://github.com/ethereum/EIPs/issues/677) but it uses the [ERC-721](./00721.md) approach, so it doesn't override the [ERC-20](./00020.md) `transfer` and `transferFrom` methods and defines the interfaces IDs to be implemented maintaining the [ERC-20](./00020.md) backwards compatibility.  

## Security Considerations
The `approveAndCall` and `transferFromAndCall` methods can be affected by the same issue of the standard [ERC-20](./00020.md) `approve` and `transferFrom` method.
