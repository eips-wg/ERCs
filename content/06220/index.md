...
The Composable NFTs utilizing equippable parts standard extends [ERC-721](../00721.md) by allowing the NFTs to selectively add parts to themselves via equipping.
...
The Equippable token standard has been made compatible with [ERC-721](../00721.md) in order to take advantage of the robust tooling available for implementations of ERC-721 and to ensure compatibility with existing ERC-721 infrastructure.

## Test Cases

Tests are included in [`equippableFixedParts.ts`](./assets/test/equippableFixedParts.ts) and [`equippableSlotParts.ts`](./assets/test/equippableSlotParts.ts).
...
See [`EquippableToken.sol`](./assets/contracts/EquippableToken.sol).
