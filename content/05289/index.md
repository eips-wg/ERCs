...
To request that certain documents be signed, revert with an [ERC-5568](../05568.md) signal. The format of the `instruction_data` is an ABI-encoded `(address, uint16)` pair, where the address is the address of the library, and the `uint16` is the `documentId` of the document:
...
![image](./assets/example-popup.png)
...
See [`IERC5289Library`](./assets/interfaces/IERC5289Library.sol), [`ERC5289Library`](./assets/ERC5289Library.sol).
