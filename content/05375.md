...
All addresses used in this standard MUST follow the casing rules described in [EIP-55](./00055.md).
...
- If a contract supports the metadata extension for [EIP-721](./00721.md), the JSON document pointed by `tokenURI(uint256 _tokenId)` MUST include the top-level field `authorInfo`
- If a contract supports the metadata extension for [EIP-1155](./01155.md), the JSON document pointed by `uri(uint256 _id)` MUST include a top-level field `authorInfo`
...
Consent is obtained by signing an [EIP-712](./00712.md) compatible message. Specifically, the structure is defined as follows:
