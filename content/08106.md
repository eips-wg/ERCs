...
This standard defines an event-based compliance framework for Real World Asset (RWA) tokens on [ERC-20](./00020.md), providing:
...
Real World Asset tokenization requires compliance observability that existing [ERC-20](./00020.md) tokens do not provide:
...
4. **Token Identification**: The `token` parameter MUST be the address of the [ERC-20](./00020.md) token contract
...
1. **Value Flows**: Every `ComplianceObserved` event corresponds to a real [ERC-20](./00020.md) balance change
...
**Event Integrity:** Implementations MUST emit `ComplianceObserved` events only after actual [ERC-20](./00020.md) balance changes. Emitting events without value movement compromises auditability.
