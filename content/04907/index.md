...
This standard is an extension of [EIP-721](../00721.md). It proposes an additional role (`user`) which can be granted to addresses, and a time where the role is automatically revoked (`expires`). The `user` role represents permission to "use" the NFT, but not the ability to transfer it or set users.
...
### Contract Interface
Solidity Interface with NatSpec & OpenZeppelin v4 Interfaces (also available at [`IERC4907.sol`](./assets/contracts/IERC4907.sol)):
...
### Test Contract
`ERC4907Demo` Implementation: [`ERC4907Demo.sol`](./assets/contracts/ERC4907Demo.sol)
...
Implementation: [`ERC4907.sol`](./assets/contracts/ERC4907.sol)
```solidity
// SPDX-License-Identifier: CC0-1.0
pragma solidity ^0.8.0;
