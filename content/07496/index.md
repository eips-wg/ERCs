...
This specification introduces a new interface that extends [ERC-721](../00721.md) and [ERC-1155](../01155.md) that defines methods for setting and getting dynamic onchain traits associated with non-fungible tokens. These dynamic traits can be used to represent properties, characteristics, redeemable entitlements, or other attributes that can change over time. By defining these traits onchain, they can be used and modified by other onchain contracts.
...
Contracts implementing this EIP MUST include the events, getters, and setters as defined below, and MUST return `true` for [ERC-165](../00165.md) `supportsInterface` for `0xaf332f3e`, the 4 byte `interfaceId` for this ERC.
...
If a `tokenId` does not exist, the contract MUST revert with `TokenDoesNotExist(tokenId)`. If the token contract already defines a custom error for non-existent tokens (e.g. from [ERC-721](../00721.md) or [ERC-1155](../01155.md)), that error MAY be used instead.
