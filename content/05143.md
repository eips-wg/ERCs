...
The following standard extends the [EIP-4626](./04626.md) Tokenized Vault standard with functions dedicated to the safe interaction between EOAs and the vault when price is subject to slippage.

## Motivation

[EIP-4626](./04626.md) security considerations section states that:
...
MUST support [EIP-20](./00020.md) `approve` / `transferFrom` on `asset` as a deposit flow.
