...
Contracts implementing this ERC MAY use hooks to redirect metadata resolution to a different contract. For the full specification, see [ERC-8121](./08121.md). When using hooks with contract metadata, the target function MUST be `getContractMetadata(string)` returning `bytes`. The hook selector is `0x9e574b14`.
