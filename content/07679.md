...
Different [ERC-4337](./04337.md) smart account implementations encode their signature, nonce, and calldata differently.  This makes it difficult for DApps, wallets, and smart account toolings to integrate with smart accounts without integrating with account-specific SDKs, which introduces vendor lock-in and hurts smart account adoption.
...
At the moment, to build a [ERC-4337](./04337.md) UserOperation (UserOp for short) for a smart account requires detailed knowledge of how the smart account implementation works, since each implementation is free to encode its nonce, calldata, and signature differently.
...
The counterfactual call contract is inspired by [ERC-6492](./06492.md), which devised a mechanism to execute `isValidSignature` (see [ERC-1271](./01271.md)) against a pre-deployed (counterfactual) contract.
