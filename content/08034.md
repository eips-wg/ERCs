...
This ERC proposes Royalty Distribution, a standalone royalty distribution for Referable Non-Fungible Tokens (rNFTs). It enables royalty distribution to multiple recipients at the primary level and referenced NFTs in the directed acyclic graph (DAG), with a single depth limit to control propagation. The standard is independent of [ERC-2981](./02981.md). and token-standard-agnostic, but expects [ERC-5521](./05521.md) rNFTs, which in practice build on [ERC-721](./00721.md) ownership semantics. It includes a function to query fixed royalty amounts (in basis points) for transparency. Royalties are voluntary, transparent, and configurable on-chain, supporting collaborative ecosystems and fair compensation.

## Motivation

[ERC-5521](./05521.md) introduces Referable NFTs (rNFTs), which form a DAG through "referring" and "referred" relationships. Existing royalty standards like [ERC-2981](./02981.md) do not account for this structure or support multiple recipients per level. This EIP addresses the need for a royalty mechanism that:
...
- Operates independently of [ERC-721](./00721.md) or [ERC-2981](./02981.md).
- Ensures transparency for marketplaces and users.
- Is discoverable via [ERC-165](./00165.md) supportsInterface.
- Supports optional [EIP-712](./00721.md) signature-based configuration to streamline marketplace or owner-driven updates.
...
The IRNFTRoyalty interface defines the royalty distribution for rNFTs and MUST inherit `ERC165` so that supporting contracts can advertise compliance via [ERC-165](./00165.md):
