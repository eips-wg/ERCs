...
|---------------------------------|---------------------|
| [EIP-155](./00155.md)         | Replay attack protection - ensures signatures include chain ID to prevent cross-chain replay attacks during agent registration and verification |
| [EIP-712](./00712.md)         | Typed structured data signing - enables human-readable signing requests for agent registration, preventing blind signing attacks and improving UX |
| [ERC-3009](./03009.md)       | Transfer with authorization - enables gasless USDC transfers for x402 micropayments, allowing verification fees without requiring ETH for gas |
| [ERC-191](./00191.md)         | Signed data standard - provides standardised format for signed messages used in wallet verification and proof validation |
...
- SHOULD use [EIP-3009](./03009.md) TransferWithAuthorization for gasless payments
...
**EIP-155 (Replay Protection)**: Agent registrations involve wallet signatures. Without chain ID inclusion ([EIP-155](./00155.md)), a registration signature on mainnet could be replayed on testnets or L2s, potentially creating conflicting agent records across chains.

**EIP-712 (Typed Data Signing)**: Registration requires users to sign structured data. [EIP-712](./00712.md) presents human-readable signing requests (e.g., "Register Agent: MyBot at 0x...") rather than opaque hashes, preventing phishing attacks where users unknowingly sign malicious transactions.

**ERC-3009 (Transfer With Authorization)**: Verification fees use x402 micropayments. [ERC-3009](./03009.md) enables gasless USDC transfers where the verification provider pays gas, improving UX by not requiring users to hold ETH for verification.

**ERC-191 (Signed Data Standard)**: Wallet verification requires proving wallet ownership. [ERC-191](./00191.md) provides the standardised prefix for signed messages, ensuring compatibility across wallets and preventing signature malleability.
...
- x402 micropayment integration with [EIP-3009](./03009.md)
