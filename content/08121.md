...
This ERC introduces hooks for cross-chain function calls. A hook fully specifies what function to call, with what parameters, on which contract, on which chain. Hooks require clients to use [ERC-3668](./03668.md) to resolve, enabling cross-chain and off-chain verifiable data resolution. Hooks are particularly useful for redirecting metadata to known contracts with verifiable security properties, such as credential registries for Proof-of-Personhood (PoP) or Know-Your-Agent (KYA) for AI agent identity.
...
- **`target`**: An [ERC-7930](./07930.md) interoperable address specifying both the target contract and chain
...
For systems that store `string` values, hooks MUST be formatted as shown below. The target is an [ERC-7930](./07930.md) interoperable address.
...
2. **Parse the hook**: Extract the `functionSelector` (if present), `functionSignature`, `functionCall`, `returnType`, and `target` ([ERC-7930](./07930.md) address). If the selector is omitted, `functionSignature` is the first parameter.
...
4. **Parse the target**: Decode the [ERC-7930](./07930.md) address to extract the chain and contract address
...
7. **Enable [ERC-3668](./03668.md)**: Clients MUST enable [ERC-3668](./03668.md) offchain data retrieval before calling the target
...
Clients MAY choose NOT to resolve hooks if the target contract is not known to be secure and trustworthy. Some clients have [ERC-3668](./03668.md) disabled by default, but clients MUST enable it before resolving the hook.
...
[ERC-7930](./07930.md) addresses include chain information, making hooks a complete cross-chain function call specification. A hook specifies exactly what function to call, with what parameters, on which contract, on which chain. This eliminates ambiguity and enables secure cross-chain reads when combined with [ERC-3668](./03668.md).
...
### Why Mandate [ERC-3668](./03668.md)?

[ERC-3668](./03668.md) (CCIP-Read) is a powerful technology that enables both cross-chain and verified offchain resolution of metadata. However, because some clients disable [ERC-3668](./03668.md) by default due to security considerations, hooks explicitly mandate [ERC-3668](./03668.md) support. This gives clients the opportunity to enable [ERC-3668](./03668.md) specifically for hook resolution without needing to have it enabled globally. By tying [ERC-3668](./03668.md) to hooks, clients can make a deliberate choice to enable it when resolving from known, trusted contracts, while keeping it disabled for general use.
