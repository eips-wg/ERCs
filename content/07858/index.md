...
Introduces an extension for [ERC-721](../00721.md) Non-Fungible Tokens (NFTs) and Soulbound Tokens (SBTs), which facilitates the implementation of an expiration mechanism.
...
- `isTokenExpired` is used for retrieving the status of the given `tokenId` the function **MUST** return `true` if the token is expired and **MUST** revert if the `tokenId` does not exist for implementation that use custom error **SHOULD** revert with `ERC721NonexistentToken` following the relevant [ERC-6093](../06093.md) and for implementation that using Solidity version below `v0.8.4` or those preferring to use revert with string error **SHOULD** revert with `NonexistToken`, If the `tokenId` exists and is not expired, the function **MUST** return `false`.
...
**Epochs** represent a specific period or block range during which certain tokens are valid borrowing concepts from [ERC-7818](../07818.md), tokens are grouped under an `epoch` and share the same `validityDuration`.
