...
Currently, in order to transfer several [ERC-721](./00721.md) NFTs, one needs to submit a number of transactions equal to the number of NFTs being tranferred. This wastes users' funds by requiring them to pay 21000 gas fee for every NFT they transfer.
...
The following JavaScript code, using the Ethers library, should atomically transfer `amt` units of an [ERC-20](./00020.md) token to both `addressA` and `addressB`.
