...
See also `https://ethereum-magicians.org/t/implementing-account-abstraction-as-part-of-eth1-x/4020` and the links therein for historical work and motivation, and [EIP-2938](../02938.md) for a consensus layer proposal for implementing the same goal.
...
* **Achieve the key goal of Account Abstraction**: allow users to use Smart Contract Accounts containing arbitrary verification logic instead of EOAs as their primary account. Completely remove any need at all for users to also have EOAs,
 as required by both status quo Smart Contract Accounts and [EIP-7702](../07702.md).
* **Decentralization**
...
* **Support other use cases**
    ...
    * Atomic multi-operations (similar goal to [EIP-7702](../07702.md))
    * Pay tx fees with [ERC-20](../00020.md) tokens, allow developers to pay fees for their users, and [EIP-7702](../07702.md)-like **sponsored transaction** use cases more generally
...
* **Bundler** - a node (block builder) that can handle `UserOperations`,
  ...
  * If the bundler is not a block builder, it MUST work with the block building infrastructure such as `mev-boost` or
    other kind of proposer-builder separation, such as [EIP-7732](../07732.md).
  * The `bundler` can also rely on an experimental `eth_sendRawTransactionConditional` RPC API defined in [ERC-7796](../07796.md) if it is available.
...
* **Aggregator** - also known as "authorizer contract" - a contract that enables multiple `UserOperations` to share a single validation, fully defined in [ERC-7766](../07766.md).
* **Canonical UserOperation mempool** - a decentralized permissionless P2P network where bundlers exchange `UserOperations` that are valid and conform with [ERC-7562](../07562.md).
* **Alternative UserOperation mempool** - any other P2P mempool where the validity of `UserOperations` is determined by rules that are different from [ERC-7562](../07562.md) in any way.
...
| `maxFeePerGas`                  | `uint256` | Maximum fee per gas (similar to [EIP-1559](../01559.md) `max_fee_per_gas`)                          |
...
Note that one [EIP-7702](../07702.md) "authorization tuple" value can be provided alongside the `UserOperation` struct,
...
* The return value MUST be packed of `aggregator`/`authorizer`, `validUntil` and `validAfter` timestamps.
  * `aggregator`/`authorizer` - 0 for valid signature, 1 to mark signature failure. Otherwise, an address of an `aggregator`/`authorizer` contract, as defined in [ERC-7766](../07766.md).
...
* **Create the `sender` Smart Contract Account if it does not yet exist**, using the `initcode` provided in the `UserOperation`.
  * If the `factory` address is "0x7702", then the sender MUST be an EOA with an [EIP-7702](../07702.md) authorization designation. The `EntryPoint` validates the authorized address matches the one specified in the `UserOperation` signature (see [Support for [EIP-7702] authorizations](#support-for-eip-7702-authorizations)).
...
The full definition of the new JSON-RPC API can be found in [ERC-7769](../07769.md).

### Support for [EIP-712](../00712.md) signatures
...
On networks with [EIP-7702](../07702.md) enabled, the `eth_sendUserOperation` method accepts an extra `eip7702Auth` parameter.
If this parameter is set, it MUST be a valid [EIP-7702](../07702.md) authorization tuple, and signed by the `sender` address.
...
Additionally, the `UserOperation` hash calculation is updated to include the desired [EIP-7702](../07702.md) delegation address.
...
In this case the `EntryPoint` doesn't hash the current [EIP-7702 delegate](../07702.md), and can be potentially executed against a modified account.
...
![](./assets/bundle-build-full-seq.svg)
...
4. Before submitting the new bundle onchain, the bundler performs the *third validation* of the entire `UserOperations` bundle. If any of the `UserOperations` fail validation, the bundler drops them, and updates their reputation, as described in  [ERC-7562](../07562.md) in detail.
...
We define `UserOperation` simulation, as the offchain view call (or trace call) to the `EntryPoint` contract with the `UserOperation`, and the enforcement of [ERC-7562](../07562.md) rules, as part of the `UserOperation` validation.
...
For the complete procedure see [ERC-7562](../07562.md)
...
The procedure of using alternate mempools is defined in [ERC-7562](../07562.md#alt-mempools-rules)
...
To prevent such rogue `UserOperations`, the bundler is required to follow a set of [restrictions on the validation function](../07562.md), to prevent such denial-of-service attacks.
