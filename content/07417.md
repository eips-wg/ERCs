...
Currently some tokens are available on different chains in different standards, for example most exchanges support [ERC-20](./00020.md) USDT, TRX USDT, BEP-20 USDT and all this tokens are in fact the same USDT token. This proposal is intended to introduce a concept where there can be a [ERC-20](./00020.md) USDT and [ERC-223](./00223.md) USDT available on Ethereum mainnet at the same time and these would be freely interchangeable.
...
As Ethereum already has an established ecosystem of tokens and [ERC-20](./00020.md) is the most adopted standard at the moment the lack of defined migration processes can be a bottleneck for newer standards adoption. This proposal addresses the problem of coordinating the upgrading process and addresses the backwards compatibility problems for [ERC-20](./00020.md) and [ERC-223](./00223.md) tokens.

The Token Converter is supposed to allow anyone to create an alternative version of an existing token implemented in a different standard. This proposal focuses on [ERC-20](./00020.md) and [ERC-223](./00223.md) standards and takes into account the specifics of this particular token standards. It is assumed that the most common case would be creation of [ERC-223](./00223.md) version for an existing [ERC-20](./00020.md) token.
...
All the [ERC-223](./00223.md) tokens created by the Token Converter will be identical in a way that they all implement the same functions, which return the same values and there is no ambiguity there. This helps to avoid problems where a token deployed during the early stage of a token standard adoption may implement it improperly or there can be an ambiguity in the standard itself that would allow developers to implement tokens of one standard in different ways. 

For example it was a common case with [ERC-20](./00020.md) where developers could implement custom logic of the `transfer` function and mess the return values. The [ERC-20](./00020.md) specification declares that a `transfer` function MUST return a `bool` value, however in practice we have three different types of [ERC-20](./00020.md) tokens which are not compatible with each other:

1. [ERC-20](./00020.md) tokens that return `true` on success and revert on an error.
2. [ERC-20](./00020.md) tokens that return `true` on success and `false` on an error without reverting the transaction.
3. [ERC-20](./00020.md) tokens that don't have return values and revert on an error.

Technically the third category of tokens is not compatible with [ERC-20](./00020.md) standard. However, USDT token deployed on Ethereum mainnet at `0xdac17f958d2ee523a2206206994597c13d831ec7` address does not implement return values and it is one of the most used tokens and it is not an option to deny supporting USDT due to it's improper implementation of the standard.
...
Returns the address of the [ERC-20](./00020.md) wrapper for a given token address. Returns `0x0` if there is no [ERC-20](./00020.md) version for the provided token address. There can be exactly one wrapper for any given [ERC-223](./00223.md) token address created by the Token Converter contract.
...
Returns the address of the [ERC-223](./00223.md) wrapper for a given token address. Returns `0x0` if there is no [ERC-223](./00223.md) version for the provided token address. There can be exactly one [ERC-223](./00223.md) wrapper for any given [ERC-20](./00020.md) token address created by the Token Converter contract.
...
Returns the address of the original [ERC-20](./00020.md) token for the provided [ERC-223](./00223.md) wrapper. Returns `0x0` if the provided `_erc223Token` is not an address of any [ERC-223](./00223.md) wrapper created by the Token Converter contract.
...
Returns the address of the original [ERC-223](./00223.md) token for the provided [ERC-20](./00020.md) wrapper. Returns `0x0` if the provided `_erc20Token` is not an address of any wrapper created by the Token Converter contract.
...
Wrapper contracts are deployed via `CREATE2` opcode and it is possible to predict the address of a wrapper which is not yet deployed. The address of a wrapper contract depends on the bytecode therefore it is necessary to specify if the address of wrapper [ERC-20](./00020.md) or wrapper [ERC-223](./00223.md) must be predicted.

Providing `_token` address and `_isERC20 = false` will result in [ERC-20](./00020.md) wrapper address being predicted.

Providing `_token` address and `_isERC20 = true` will result in [ERC-223](./00223.md) wrapper address being predicted.
...
Creates a new [ERC-223](./00223.md) wrapper for a given `_erc20Token` if it does not exist yet. Reverts the transaction if the wrapper already exist. Returns the address of the new wrapper token contract on success.  Reverts if `_erc223Token` is a wrapper created by the Converter.

The deployed contract will be a standard [ERC-223](./00223.md) token with `approve` and `transferFrom` functions implemented for backwards compatibility.

All [ERC-223](./00223.md) wrappers deployed by the Converter will have `standard() pure returns (bytes32)` function implemented which returns `223`. This serves further token standard introspection as [ERC-165](./00165.md) may not be reliable when dealing with identifying the internal logic implemented within `transfer` function of a token.

NOTE: This function does not verify the standard of `_erc20Token` because there is no reliable method of introspection available which could guarantee that the provided token implements a particular standard. As the result it is possible to create a [ERC-223](./00223.md) wrapper for an original [ERC-223](./00223.md) token.
...
Creates a new [ERC-20](./00020.md) wrapper for a given `_erc223Token` if it does not exist yet. Reverts the transaction if the wrapper already exist. Returns the address of the new wrapper token contract on success. Reverts if `_erc223Token` is a wrapper created by the Converter.

NOTE: This function does not verify the standard of `_erc223Token` because there is no reliable method of introspection available which could guarantee that the provided token implements a particular standard. As the result it is possible to create a [ERC-20](./00020.md) wrapper for an original [ERC-20](./00020.md) token.
...
There is no special function to unwrap [ERC-223](./00223.md) wrappers to [ERC-20](./00020.md) origin as this logic is implemented in the `tokenReceived` function of the Converter.
...
Automatically determines if the provided [ERC-20](./00020.md) token is a wrapper or not. If it is a wrapper then executes `unwrapERC20toERC223` function. If the provided token is an origin then executes `wrapERC20toERC223` function.
...
This is a standard [ERC-223](./00223.md) transaction handler function and it is called by the [ERC-223](./00223.md) token contract when `_from` is sending `_value` of [ERC-223](./00223.md) tokens to `address(this)` address. In the scope of this function `msg.sender` is the address of the [ERC-223](./00223.md) token contract and `_from` is the sender of the token transfer.
...
If `msg.sender` is not an address of any [ERC-223](./00223.md) wrapper known to the Token Converter then it is considered a [ERC-223](./00223.md) origin and `_value` amount of [ERC-20](./00020.md) wrapper tokens must be sent to the `_from` address. If the [ERC-20](./00020.md) wrapper for the `msg.sender` token does not exist then create it first.
...
### Converting [ERC-223](./00223.md) wrapper tokens back to [ERC-20](./00020.md)
...
### Support of [ERC-223](./00223.md) original tokens

Two methods of implementing a Token Converter service were considered: (1) a converter that can only create [ERC-223](./00223.md) versions of the existing [ERC-20](./00020.md) tokens, and (2) a converter that can create both versions ([ERC-20](./00020.md) and [ERC-223](./00223.md)) of any original token.

The first approach would encourage developers to always deploy an original token as [ERC-20](./00020.md) and then create it's [ERC-223](./00223.md) version in the converter. If it would happen that some developers may consider [ERC-223](./00223.md) as their original standard then they would be left with the problem of creating their custom [ERC-20](./00020.md) version of the token. In addition, if any third party contracts like liquidity pools are using the proposed Token Converter to ensure that a token can be listed on a DEX with two versions and both can be combined within one pool - then such contract would not be able to recognize any original [ERC-223](./00223.md) token and it's [ERC-20](./00020.md) version as a valid pair of contracts that represent one token available in two standards.

For that reason it was decided to go with the second approach where the Converter can create [ERC-20](./00020.md) wrappers for original [ERC-223](./00223.md) tokens.

### Support of `approve` & `transferFrom` functions in the [ERC-223](./00223.md) wrapper tokens

This functions are superfluous for a [ERC-223](./00223.md) token since the `transfer` function can be used to deposit tokens of this standard to contracts. The current ecosystem is built for [ERC-20](./00020.md) tokens however and there are plenty of multisig contracts that rely on accepting tokens deposited without any callback with an assumption that it is not necessary for a multisig to count the amount of tokens it stores.

There can be any other contracts and scenarios where it would be necessary to deposit a token to a contract which is relying on an assumption that tokens are deposited without invoking a callback in the recipient. As the result we can expect that any original deployed [ERC-223](./00223.md) tokens will support this functions, as token developers strive for backward compatibility with the existing ecosystem. In order to make tokens deployed by the converter a reference implementation for developers that can be used without any modifications it was decided to support this functions in the [ERC-223](./00223.md) wrapper contracts.
...
As the result, the `approve` & `transferFrom` transferring method must be avoided with [ERC-223](./00223.md) tokens whenever possible.

### Modified transfer events of the [ERC-223](./00223.md) token

The pure [ERC-223](./00223.md) token implementation has the following event emitted on a token transfer: `event Transfer(address indexed _from, address indexed _to, uint256 _value, bytes _data)`. This events are different from ones emitted by [ERC-20](./00020.md) tokens and may not be properly recognized by existing blockchain explorers, wallets and other services that browse token transfers history.
...
The main existing method of introspection is currently [ERC-165](./00165.md) which inspects the signatures of functions implemented in a contract. It is not possible to differentiate an [ERC-20](./00020.md) token from an [ERC-223](./00223.md) token by just browsing functions that they implement without digging their internal logic.

Here is a token and it is not possible to identify if it should be dealt with as [ERC-20](./00020.md) or [ERC-223](./00223.md) because it depends on the actual implemenation of it's `transfer` function logic.
...
In case of this implementation the token will behave as [ERC-20](./00020.md):
...
In case of this implementation the token will behave as [ERC-223](./00223.md):
...
Also, there are plenty of tokens that do not implement [ERC-165](./00165.md) introspection at all. As the result it was decided to implement a special `standard() returns (uint32)` function in all [ERC-223](./00223.md) wrappers created by the Converter and assume that original [ERC-223](./00223.md) tokens may explicityly declare themselves as [ERC-223](./00223.md) by implementing the same function too. It is assumed that if a token does not implement this function then it is [ERC-20](./00020.md).

This method of token standard introspection is more precise than [ERC-165](./00165.md).
