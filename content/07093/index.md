...
![social_recovery_flow](./assets/social-recovery-flow.svg)
...
- For the `Guardian`'s signable message, it SHOULD employ [EIP-712](../00712.md) type signature to ensure the content of the signature is readable and can be confirmed accurately during the Guardian signing process.
...
2. When GuardianA and GuardianB assist the user in performing account recovery, they are to confirm the [EIP-712](../00712.md) structured data for signing, which might look like this:
...
4. `startRecovery()` function's processing logic is as follows:
   ...
   - Generate a message hash (`msgHash`) from the input parameters `0xA`, `newOwners` and internally generated [EIP-712](../00712.md) signature parameters and `RecoveryNonce`.
