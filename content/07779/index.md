...
This proposal outlines the interfaces to make delegated EOAs interoperable after the merge of [EIP-7702](../07702.md). With [EIP-7702](../07702.md), EOAs will be able to enable execution abstraction, which leads to a more feature-rich account, including gas sponsorship, batch execution, and more.
...
After the merge of [EIP-7702](../07702.md), it is expected that a considerable number of EOA wallets will migrate from pure EOA accounts to delegated EOA accounts.
...
However, after the merge of [EIP-7702](../07702.md), each EOA will be given the ability to delegate itself to a smart account which will impact migration as storage remains in the continuous context while EOA can be delegated to diverse smart accounts if the user migrates their wallet.
...
Moreover, it is worth noting that this spec is not limited to [EIP-7702](../07702.md) based smart accounts but smart accounts and smart contracts in general that uses a custom storage slot.
...
![diagram showing the flow of onredelegation](./assets/diagram.svg)
...
In fact, this standard thinks exactly the opposite. It is worth scanning the storage, or at least the storage that the delegated account will use, which the wallet wants to delegate to. E.g., Just like developers validating the storage of Facets in Diamond ([ERC-2535](../02535.md)) to prevent storage collision and not just relying on hash probability.
...
Existing smart accounts that was built prior to the [EIP-7702](../07702.md) discussion will need changes to support this standard.
