...
The interface identifier for `IERC7802` is **`0x33331994`**, calculated according to [ERC-165](./00165.md) as the XOR of the function selectors of the two functions in the interface:
...
Implementations SHOULD emit `Transfer(address(0), _to, _amount)` on calls to `crosschainMint` to be compliant with [ERC-20](./00020.md) invariants on token creation.
...
Implementations might consider emitting `Transfer(_from, address(0), _amount)` on calls to `crosschainBurn` to be compliant with [ERC-5679](./05679.md).
...
- Integrations with Different Chains: To make an [ERC-20](./00020.md) cross-chain compatible,
issuers simply need to implement the [ERC-7802](./07802.md) extension with the corresponding access controls for each chain. 
...
This proposal is fully backwards compatible with [ERC-20](./00020.md).
