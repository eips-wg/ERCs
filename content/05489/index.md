...
This EIP proposes a new extension for NFTs (non-fungible token, aka [EIP-721](../00721.md)): nft-hyperlink-extention (hNFT), embedding NFTs with hyperlinks, referred to as “hNFTs”. As owners of hNFTs, users may authorize a URL slot to a specific address which can be either an externally-owned account (EOA) or a contract address and hNFT owners are entitled to revoke that authorization at any time. The address which has slot authorization can manage the URL of that slot.
...
You can find an implementation of this standard in [`ERC5489.sol`](./assets/contracts/ERC5489.sol).
