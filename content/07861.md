...
This standard is an extension of [ERC-721](./00721.md) that adds verifiable credential properties at the contract level. A verifiable credential is a secure digital certificate that encapsulates claims issued by a trusted authority and is designed to be tamper-evident and readily verifiable. It allows [ERC-721](./00721.md) tokens to natively reference and manage these credentials—as well as other types of off-chain data—by defining mechanisms for storage, encryption, revocation, and verification.

## Motivation
Many [ERC-721](./00721.md) contracts require additional properties to represent verifiable credentials. While it's possible to implement these properties in various ways, it creates an extra effort for third-party platforms to build individualized solutions for each NFT collection.
...
The `supportsInterface` [ERC-165](./00165.md) method MUST return true when called with `0x7861069`.
...
This standard is compatible with current [ERC-721](./00721.md) standards. It adds new functions without modifying the existing [ERC-721](./00721.md) functionality.
