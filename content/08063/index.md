...
Following the [ERC-20](../00020.md) pattern, each group is its own contract deployment:
...
- Resource associations: External contracts (e.g., [ERC-20](../00020.md), [ERC-721](../00721.md)) MAY track group ownership or membership-based access using the group's contract address.
...
Implementations MAY expose an [ERC-20](../00020.md) interface for group membership under the following constraints:
...
- **One contract per group**: Following the [ERC-20](../00020.md) model where each token is its own contract creates a simpler, more composable design. The contract address becomes the natural group identifier.
...
- **Relationship to [ERC-7743](../07743.md)**: MO-NFT models multi-ownership of a single token; Groups model multi-membership of a container. Implementations MAY associate a Group with tokens, but the standard does not require token interfaces.

## Backwards Compatibility

No known backward compatibility issues. This is a new interface with [ERC-165](../00165.md) detection.
