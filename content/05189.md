...
* **Support other use cases:**
  ...
  * Atomic multi-operations (similar to [EIP-3074](./03074.md)).
  * Payment of transaction fees using tokens. (E.g. [ERC-20](./00020.md), [ERC-777](./00777.md), etc.)
...
| maxFeePerGas               | uint256 | Max amount of basefee that the `operation` execution is expected to pay. _(Similar to [EIP-1559](./01559.md) `max_fee_per_gas`)_.                        |
| priorityFeePerGas          | uint256 | Fixed amount of fees that the `operation` execution is expected to pay to the bundler. _(Similar to [EIP-1559](./01559.md) `max_priority_fee_per_gas`)_. |
...
The `endorser` MAY use the `simulationSettings` method to provide a list of replacements that the bundler should apply to the network before simulating `isOperationReady`. This is useful for operations that must be called from specific contract addresses or that depend on specific storage values (e.g. [ERC-4337](./04337.md)'s EntryPoint).
...
The payment is always made in the `feeToken`, which can be any token standard (E.g. [ERC-20](./00020.md)). If `feeToken` is `address(0)`, then payment is made in the native currency. When `feeToken` is `address(0)`, `feeScalingFactor` and `feeNormalizationFactor` MUST be equal to `1`.
