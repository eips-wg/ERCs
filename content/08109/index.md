...
![Diagram showing how a diamond contract works](./assets/basic-diamond-diagram.svg)

Diamond contracts were originally standardized by [ERC-2535](../02535.md). This standard refines that specification by simplifying terminology, reducing the implementation complexity of introspection functions, and standardizing specific events.
...
Additional motivation and background for diamond-based smart-contract systems can be found in [ERC-1538](../01538.md) and [ERC-2535](../02535.md).
...
ERC-2535 standardized introspection functions and events. [ERC-8109](../08109.md) re-standardizes these to make diamond contracts easier to implement and easier to understand.
...
![Diagram showing structure of a diamond](./assets/functionFacetMapping.svg)
...
A reference implementation exists for these introspection functions here: [DiamondInspectFacet.sol](./assets/DiamondInspectFacet.sol)
...
1. **Optional, gas-optimized functionality**  
   ...
   For example, an [ERC-721](../00721.md) diamond may implement batch transfer functions in a dedicated facet, improving both gas efficiency and usability while keeping the base ERC-721 implementation simple and well-scoped.
...
Diamonds and facets need to use a storage layout organizational pattern because Solidity’s default storage layout doesn’t support proxy contracts or diamonds. The storage layout technique or pattern to use is not specified in this ERC. However, examples of storage layout patterns that work with diamonds are [ERC-8042 Diamond Storage](../08042.md) and [ERC-7201 Namespaced Storage Layout](../07201.md).
...
- The reference implementation for the `facetAddress(bytes4 _functionSelector)` and `functionFacetPairs()` introspection functions is here: [DiamondInspectFacet.sol](./assets/DiamondInspectFacet.sol)
- An example implementation of an ERC-8109 diamond is here: [DiamondExample.sol](./assets/DiamondExample.sol)
