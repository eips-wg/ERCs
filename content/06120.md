...
The default transaction behavior of ETH is *transfer-and-call*, but the widely used [ERC-20](./00020.md) standard isn't compatible with this pattern. This incompatibility forces applications to use an inefficient and risky two-step *approve-then-call* process. This approach is costly, creates a poor user experience, and introduces significant security vulnerabilities, as users must approve unaudited and often upgradable contracts. This has led to numerous allowance-related bugs and exploits.
...
A special id `ERC_721_BALANCE` is reserved for [ERC-721](./00721.md), which can be used in output actions to verify the total amount of all ids owned by the `recipient` address.
...
Old token contracts (ERC-20, ERC-721 and [ERC-1155](./01155.md)) require approval for the Universal Token Router once for each account.
