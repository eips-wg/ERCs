...
## Abstract
It has been a significant challenge for developers attempting to create cloneable and upgradeable contracts on the Ethereum Virtual Machine (EVM). While [ERC-2535](../02535.md) Diamonds and other existing proxy standards offer partial solutions, a comprehensive answer has remained elusive. Our proposal addresses this gap through the introduction of two main features.

### Function-Level Upgradeability
In alignment with [ERC-2535](../02535.md), this functionality permits the selective redirection of implementation contracts for individual function calls. This granular control over upgrades allows for modifications on a per-function basis. Moreover, segmenting implementation contracts by function helps mitigate the limitations posed by the contract size cap (24.576kB as of EVM version Shanghai or earlier).

### Factory/Clone-Friendly & Simultaneous Upgradeability
Drawing on the Beacon model from [ERC-1967](../01967.md), our method aims to streamline the process of cloning and updating Proxy contracts simultaneously. This approach is designed to maintain consistent functionality across different instances, each with its own state. Typically, proxies are limited to basic upgradeability features or follow the [ERC-1167](../01167.md) standard. However, our solution combines both functionalities into a compact proxy.
...
2. When a Factory is needed without Upgradeability, [ERC-1167](../01167.md) is suitable.
3. For Contract-Level Upgradeability without a Factory, [ERC-1822](../01822.md) can be used.
4. For Contract-Level Upgradeability with a Factory, the Beacon from [ERC-1967](../01967.md) is applicable.
5. For Function-Level Upgradeability without a Factory, [ERC-2535](../02535.md) is available.
6. For Function-Level Upgradeability with a Factory, this ***Upgradeable Clone Standard*** is the ideal choice.

![Fig. Use Cases](./assets/images/usecases.svg)
...
#### Storage & Events
This contract SHOULD store the _Dictionary Contract_ address in the storage slot `0x267691be3525af8a813d30db0c9e2bad08f63baecf6dceb85e2cf3676cff56f4`, obtained as `bytes32(uint256(keccak256('erc7546.proxy.dictionary')) - 1)`, in accordance with the method defined in [ERC-1967](../01967.md). This ensures that the address is stored in a secure and predictable slot.
...
##### `supportsInterface`
This contract is RECOMMENDED to implement the `supportsInterface(bytes4 interfaceID)` function defined in [ERC-165](../00165.md) to indicate which interfaces are supported by the contracts referenced in the mapping.
...
For instance, the storage could be arranged according to useful storage layout patterns, such as ***[ERC-7201](../07201.md)***.
...
![Fig. Comparison with Beacon](./assets/images/comparison-with-beacon.svg)
...
- Reference Implementations
  - [Proxy Contract](./assets/src/Proxy.sol)
  - [Dictionary Contract](./assets/src/Dictionary.sol)
- Tests
  - [Proxy Spec Test](./assets/test/Proxy.spec.t.sol)
  - [Dictionary Spec Test](./assets/test/Dictionary.spec.t.sol)
  - [UCS Usecase Test](./assets/test/UCS.usecase.t.sol)
