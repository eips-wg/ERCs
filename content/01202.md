...
4. Allow the compatibility with token standard such as [ERC-20](./00020.md) or other new standards([ERC-777](./00777.md)) and item standard such as [ERC-721](./00721.md)
5. Create massive potential for interoperability within Ethereum echo systems and other system.
6. Allow setting voting deadline, allow determine on single or multiple options. Allow requiring voting orders. (trade-off is interface complexity, we might need [ERC-20](./00020.md) approach and later a [ERC-777](./00777.md) for advanced voting)
...
whether they are on-chain or off-chain and whether they are exeutable, is leaved out from this proposal. A separate EIP could be proposed to address this particular use case. See one of such proposals [ERC-5247](./05247.md)
...
account to generate the transaction, the signature aggregations or endorsements are not in scope of this EIP. A separate EIP could be proposed to address this particular use case. See one of such proposals here [ERC-5453](./05453.md).

### Use-cases

1. Determine on issuing new token, issuing more token or issuing sub-token
2. Determine on creating new item under [ERC-721](./00721.md)
...
3. the compliant contract SHOULD implement [ERC-5269](./05269.md) interface.
