...
This standard is an extension of [EIP-721](./00721.md). It introduces lockable NFTs. The locked asset can be used in any way except by selling and/or transferring it. The owner or operator can lock the token. When a token is locked, the unlocker address (an EOA or a contract) is set. Only the unlocker is able to `unlock` the token.
...
When the token is locked, all the [EIP-721](./00721.md) transfer functions MUST revert, except if the transaction has been initiated by an unlocker.
When the token is locked, the [EIP-721](./00721.md) `approve` method MUST revert for this token.
When the token is locked, the [EIP-721](./00721.md) `getApproved` method SHOULD return `unlocker` address for this token so the unlocker is able to transfer this token.
...
This standard is compatible with current [EIP-721](./00721.md) standards.
...
There are no security considerations related directly to the implementation of this standard for the contract that manages [EIP-721](./00721.md) tokens.
