...
Compliant tokens MUST implement [ERC-165](./00165.md). The `supportsInterface` function MUST return `true` if `0x4958f2a4` is passed through the `interfaceID` argument.
...
- #### `contractURI()`
  ...
  Returns the metadata URI for the token. SHOULD follow the schema defined in [ERC-7572](./07572.md).
