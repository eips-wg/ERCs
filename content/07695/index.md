...
This standard is an extension for [ERC-721](../00721.md), designed to specify users for various contexts with a locking feature and allow temporary ownership delegation without changing the original owner.
...
For standard [ERC-721](../00721.md) NFTs, there are several use cases in financial applications, including:
...
Smart contracts implementing this standard MUST implement the [ERC-165](../00165.md) `supportsInterface` function and MUST return the constant value `true` if `0x486b6fba` is passed through the `interfaceID` argument.
...
In addition to extending from [ERC-721](../00721.md) for the transfer mechanism when transferring an NFT, the implementation:
...
This proposal is backward compatible with [ERC-721](../00721.md).
...
![Mortgage Flow](./assets/mortgage.svg)
