...
With the inheritance of [ERC-20](../00020.md) functionalities, token holders enjoy flexibility in trading tokens on secondary markets, and an optional burning mechanism empowers them to actively contribute to a deflationary economic model while obtaining a proportional share of the revenue pool.
...
**Every compliant contract must implement the `IERC7641`, and [ERC-20](../00020.md) interfaces.**
...
The **AltRevToken extension** is OPTIONAL for this standard. This allows the contract to accept other [ERC-20](../00020.md) revenue tokens (more than ETH) into the revenue sharing pool.

The AltRevToken extension
- Defines a `claimableERC20` function to calculate the amount of [ERC-20](../00020.md) claimable by a token holder at certain snapshot.
- Defines a `redeemableERC20OnBurn` function to calculate the amount of [ERC-20](../00020.md) redeemable by a token holder upon burn.
...
where `totalSupply` denotes the total supply of [ERC-7641](../07641.md) token, and `totalRedeemableETH` denotes the total amount of ETH in the burning pool.
...
- [ERC-7641](./assets/contracts/ERC7641.sol)
